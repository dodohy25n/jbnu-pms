...
public class TaskFileService {
         ...
         * 태스크 파일 다운로드
         */
        public Resource downloadTaskFile(Long taskId, Long fileId, Long userId) {
                // 태스크 존재 확인
                Task task = taskRepository.findById(taskId)
                                .orElseThrow(() -> new CustomException(ErrorCode.TASK_NOT_FOUND));
                ...
                // S3에서 파일 스트림 가져오기
                log.info("태스크 파일 다운로드 요청: fileId={}, taskId={}, userId={}", fileId, taskId, userId);
                return s3FileService.downloadFile(taskFile.getFileUrl());
        }
